...
fn single_thread(c: &mut Criterion) {
  let mut g = c.benchmark_group("instant/single_thread");

  let now = Instant::now();
  let next = now + Duration::from_secs(1);

  let v = AtomicInstant::new(now);
  g.bench_function("AtomicInstant/load", |b| {
    b.iter(|| black_box(v.load(Ordering::Acquire)))
  });
  g.bench_function("AtomicInstant/store", |b| {
    b.iter(|| v.store(black_box(next), Ordering::Release))
  });

  let v = AtomicOptionInstant::new(Some(now));
  g.bench_function("AtomicOptionInstant/load", |b| {
    b.iter(|| black_box(v.load(Ordering::Acquire)))
  });
  g.bench_function("AtomicOptionInstant/store", |b| {
...
fn contended_read(c: &mut Criterion) {
  let mut g = c.benchmark_group("instant/contended_read");

  let now = Instant::now();

  let v = Arc::new(AtomicInstant::new(now));
  ...
  let v = Arc::new(AtomicOptionInstant::new(Some(now)));
  let (stop, handles) = bg_threads(THREADS, {
    let v = v.clone();
    move || { black_box(v.load(Ordering::Acquire)); }
  });
...
fn load_under_write_contention(c: &mut Criterion) {
  let mut g = c.benchmark_group("instant/load_under_write_contention");
...
fn contended_store(c: &mut Criterion) {
  let mut g = c.benchmark_group("instant/contended_store");
  ...
  g.bench_function("AtomicInstant/swap", |b| {
